...
print("Aquest programa fa servir una array de spins -1 sobre el qual s'hi afegeixen columnes de l'spin contrari com a situacio inicial")
percent=int(input("Quin percentatge de la configuració inicial estarà amb spin +1?:"))
...
#no escric les columnes com a numero directament sino que l'hi assigno un nom per despres poder passar les columnes per un for
col=100
config=np.zeros((100,col), dtype=int)-1
#creo 100 valors possibles, un per a cada columna
columnes=list(range(1,101))
#agafo numeros de columnes aleatories que es corresponguin a el % desitjat
seleccionades=random.sample(columnes, percent)
for col in seleccionades:
    config[:,col-1]=1 #canvia el valor de la columna seleccionada a 1
print(config)
